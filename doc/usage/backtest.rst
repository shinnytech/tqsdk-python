...
示例::
    ...
    api = TqApi(backtest=TqBacktest(start_dt=date(2026, 5, 18), end_dt=date(2026, 5, 22)), auth=TqAuth("快期账户", "账户密码"))
    ...
    # 预期输出：
    # 2026-05-18 09:30:00.000000 CFFEX.T2606
...
回测框架的规则是当没有新的事件需要用户处理时才推进到下一个行情, 也就是这样::
  ...
  api.insert_order("SHFE.cu2607", ...)  # 程序下单
  api.wait_update()                     # 这个 wait_update 只会更新委托单状态, 行情还是停在原处
  api.insert_order("SHFE.cu2607", ...)  # 如果又下了一个单
...
如果您想要在回测中同时交易期货和股票合约，则可以使用 :py:class:`~tqsdk.TqMultiAccount` 来实现该需求::
    ...
    # 多账户下单，需要指定下单账户
    order1 = api.insert_order(symbol="SHFE.cu2607", direction="BUY", offset="OPEN", volume=10, limit_price=72250.0, account=tqsim_future)
...
TqSdk 允许一个策略程序中使用多个行情序列, 比如这样::
  ...
  #... 策略程序代码
  ka1 = api.get_kline_serial("SHFE.cu2607", 60)
  ka2 = api.get_kline_serial("SHFE.cu2607", 3600)
  kb  = api.get_kline_serial("CFFEX.IF2606", 3600)
  tsa  = api.get_tick_serial("CFFEX.IF2606")
  qa = api.get_quote("DCE.a2609")
  #... 策略程序代码
...
规则2: K线序列 (例如上面例子中的ka1, ka2) 总是按周期推进. 每根K线在创建时和结束时各更新一次::
  ...
  ka2 = api.get_kline_serial("SHFE.cu2607", 3600) # 请求小时线
...
规则4: 策略程序中的多个序列的更新, 按时间顺序合并推进. 每次 wait_update 时, 优先处理用户事件, 当没有用户事件时, 从各序列中选择下一次更新时间最近的, 更新到这个时间::
  ...
  ka = api.get_kline_serial("SHFE.cu2607", 10)              # 请求一个10秒线
  kb = api.get_kline_serial("SHFE.cu2608", 15)              # 请求一个15秒线

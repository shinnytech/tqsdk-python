...
用快期账户来模拟交易
-------------------------------------------------
注册完成的快期账户的【手机号】/【邮箱地址】/【用户名】和【密码】可以作为 快期模拟 账号，通过 :py:class:`~tqsdk.TqKq` 对 auth 传入参数进行登录。这个 快期模拟 账户在快期 APP、快期专业版、快期 v2、快期 v3 和天勤量化上是互通的；程序中使用 :py:class:`~tqsdk.TqKq` 产生的模拟持仓、委托和成交记录，也可以在这些客户端中查看，示例::

...
class TqEdbIndexDataFrame(DataFrame):
    """
    EDB 指标取值查询结果（不更新）。
    行索引为日期字符串 YYYY-MM-DD，列为指标 id
    """

    def __init__(self, api, ids, start: str, end: str, align: Optional[str] = None, fill: Optional[str] = None):
        self.__dict__["_api"] = api
        ...
        self.__dict__["_payload"] = {"ids": ids, "start": start, "end": end}
        self.__dict__["_url"] = "https://edb.shinnytech.com/data/index_data"
        super(TqEdbIndexDataFrame, self).__init__(data=[], columns=ids)
    ...
    async def async_update(self):
        ...
        async with self.__dict__["_api"].register_update_notify(finished) as update_chan:
            async for _ in update_chan:
                ...
                pos_map = {k: i for i, k in enumerate(ids_from_server)}
                dates = []
                rows = []
                for dt in sorted(values.keys()):
                    ...
                    if row is None:
                        continue
                    dates.append(dt)
                    rows.append([row[pos_map[i]] if (i in pos_map and pos_map[i] < len(row)) else None for i in requested_ids])

                df = DataFrame(rows, index=Index(dates, name=self.index.name), columns=requested_ids)

                # 对齐/填充（仅 align="day" 时生效）
                if self.__dict__.get("_align") == "day":
                    ...
                    if start_s and end_s:
                        ...
                        df = df.reindex(Index(all_dates, name=df.index.name))
                    fill = self.__dict__.get("_fill")
                    if fill == "ffill":
                        df = df.infer_objects().ffill()
                    elif fill == "bfill":
                        df = df.infer_objects().bfill()

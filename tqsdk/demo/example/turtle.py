...
class Turtle:
    ...
    def recalc_paramter(self):
        if not self.klines.is_ready():
            return False
        df = self.klines.to_dataframe()
        # 平均真实波幅(N值)
        self.n = ATR(df, self.atr_day_length)["atr"].iloc[-1]
        # 买卖单位
        ...
        logger.info("唐其安通道上下轨: %f, %f" % (self.donchian_channel_high, self.donchian_channel_low))
        return True
        ...
        while self.state["position"] == 0:
            ...
            if self.api.is_changing(self.quote, "last_price"):
                ...
                if self.quote["last_price"] > self.donchian_channel_high:  # 当前价>唐奇安通道上轨，买入1个Unit；(持多仓)
                    logger.info("当前价>唐奇安通道上轨，买入1个Unit(持多仓): %d 手" % self.unit)
                    ...
                    logger.info("当前价<唐奇安通道下轨，卖出1个Unit(持空仓): %d 手" % self.unit)
                    self.set_position(self.state["position"] - self.unit)

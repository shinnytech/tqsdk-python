...
class TargetPosTask(object):
    ...
    def __init__(self,api,symbol,price="ACTIVE",trade_chan=None,settle_yesterday_first=True):
        """
        创建目标持仓task实例
        Args:
        ...
        self.symbol = symbol
        self.price = price
        self.init_pos = self._update_position()
    ...
    def _update_position(self):
        ...
        starting_pos=self.current_position['volume_long_today']+self.current_position['volume_long_his']-self.current_position[
            'volume_short_his']-self.current_position['volume_short_today']
        return starting_pos
    ...
    def shfe_settle(self,delta_volume):
        ...
        target_direction_type="BUY" if delta_volume>0 else "SELL"  #净持仓增加为买平空，否则为卖平多
        operations_to_go=[]
        self._update_position()
        position_status=self.current_position
        #昨仓今仓多空可用
        ...
        if secondary_position>0:  #优先仓不够平
            secondary_operation=InsertOrderUntilAllTradedTask(self.api,self.symbol,target_direction_type,
                                                              secondary_offset,secondary_position,price=self.price,
                                                              trade_chan=self.trade_chan)
            prior_operation=InsertOrderUntilAllTradedTask(self.api,self.symbol,target_direction_type,prior_offset,
                                                          prior_available_position,price=self.price,
                                                          trade_chan=self.trade_chan)
            ...
            operations_to_go.append(
                InsertOrderUntilAllTradedTask(self.api,self.symbol,target_direction_type,prior_offset,abs(delta_volume),
    ...
    def open_position(self,delta_volume):
        ...
        insert_order_until_all_traded_task=InsertOrderUntilAllTradedTask(self.api,self.symbol,
                                                                         "BUY" if long_short else "SELL","OPEN",
                                                                         abs(delta_volume),price=self.price,
                                                                         trade_chan=self.trade_chan)
    ...
    async def _target_pos_task(self):
        ...
        async for target_pos in self.pos_chan:
            ...
            if (0<=target_pos<current_pos) or (0>=target_pos>current_pos):  #减多仓或者减空仓
                operation_list_to_go.extend(self.settle_position(delta_volume))
